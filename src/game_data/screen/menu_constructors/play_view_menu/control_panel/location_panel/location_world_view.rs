...
use crate::game_data::{game_event_manager::prelude::Event, locations::world_area::WorldArea, player_data::{locations::location::WorldLocation, player_data::PlayerData}, screen::{menu_constructors::play_view_menu::control_panel::view_panel_input_constructor, widget::{panel::panel::{Panel, PanelAlignment, PanelOrientation}, widget_calculations::TextSize, world_rendering::play_world_view_config::PlayViewRendingConfig}}, types::UITextures};

pub fn add_location_world_view_panel(panel: &mut Panel, player_data: &mut PlayerData, location_manager_location: Rc<RefCell<WorldLocation>>) {
    ...
    // Header
    let text_display = play_view_sub_panel.add_text_display("Location Manager".to_string());
    text_display.set_text_scale(TextSize::Medium);
        

    let rendering_config = PlayViewRendingConfig::new(player_data.get_world_ref(), location_manager_location);
