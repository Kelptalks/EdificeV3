...
use crate::game_data::{TextureManager, game_event_manager::{game_event_manager::GameEventManager, render_event_manager::render_event_manager::RenderEvent, world_event_manager::world_event_manager::WorldEvent}, level_manager::level, screen::{Button, ScreenData, render_centered_string_at_ndc}, types::UITextures};
...
impl LevelSelectMenu {
    ...
    pub fn window_resize_update(&mut self, screen_data: &ScreenData) {
        ...
        //
        let scale = 0.1;
        let button_spacing = scale / 2.0;
        let total_button_spacing = scale + button_spacing;
        ...
        if mouse_button == MouseButton::Left {
            ...
            for button in &mut self.buttons_levels {
                ...
                if button.is_mouse_on_button() {
                    ...
                    event_manager.add_render_event(RenderEvent::InitWorldRender(10));
                    event_manager.add_render_event(RenderEvent::ChangeMenu(crate::game_data::screen::screen_data::CurrentMenu::Camera));
                }
                level_index += 1;
            }
        }
