...
use crate::game_data::{TextureManager, World, game_event_manager::game_event_manager::GameEventManager, screen::{ScreenData, camera_data::Direction, iso_cord_tool, widget::{panel::panel::Panel, widget::Widget, world_rendering::play_block::PlayBlock}}, types::BlockTexture};
...
impl PlayWorldViewRender {
    pub fn new(parent_pos: [f32; 4], buffers: [f32; 4]) -> PlayWorldViewRender{
        let mut panel = Panel::new(parent_pos, buffers);
        panel.add_text_display("Play View".to_string());
        PlayWorldViewRender {
            panel: panel,
    ...
    pub fn render_view(&mut self, 
        ...
        let ndc_x_draw_center_offset = self.ndc_block_scale;
...
impl Widget for PlayWorldViewRender {
    ...
    fn get_prefered_scale(&self) -> [f32; 2] {
        return self.panel.get_prefered_scale();
    }
    ...
    fn size(&mut self) {
        ...
        // self.prefered_scale = [self.ndc_block_scale * self.zoom as f32 * 2.0; 2];
    }
