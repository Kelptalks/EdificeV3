...
use crate::game_data::{World, game_event_manager::{event_manager::{Event, EventManager}, game_event_manager::GameEvent, render_event_manager::{render_event_manager::RenderEvent, window_manager_event::WindowManagerEvent}}, player_data::game_entity::{block_entity_manager::{self, natural::flungle::BlockEntityFlungle, player_created::{battery::{BatteryEvent, BlockEntityBattery}, radar::{BlockEntityRadar, RadarEvent}}}, game_entity_manager::{GameEntity, GameEntityEvent, GameEntityId}, components::{entity_components::EntityComponent, powered_component::PoweredComponentEvent}}, screen::widget::{self, panel::panel::Panel, widget::{Widget, WidgetType}, window_manager::{window::WidgetWindow, windows::window_type::{Window, WindowType}}}, tik_manager::game_time::GameTime};
    ...
    pub fn tik(&mut self, time: &GameTime, world: &World, event_manager: &mut EventManager) {
        match self {
            BlockEntity::Radar(radar) => {
                radar.tik(time, world, event_manager);
            },
            BlockEntity::Battery(battery) => {
                battery.tik(time, world, event_manager);
            },
            BlockEntity::Flour(flungle) => {
                flungle.tik(time, world, event_manager)
            },
        }
    }
}
...
impl BlockEntityManager {
    ...
    pub fn add_object(&mut self, new_block_entity: BlockEntity) {
        match new_block_entity {
            ...
            BlockEntity::Battery(battery) => {
                self.batterys.insert(battery.id, battery);
            },
            BlockEntity::Flour(flungle) => {
                self.flungles.insert(flungle.id, flungle);
            },
        }
    }
